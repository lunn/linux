...
static int mv88e6095_tag_remap(struct mv88e6xxx_chip *chip, int port)
{
	int err;
...
static int mv88e6390_tag_remap(struct mv88e6xxx_chip *chip, int port)
{
	int err, reg, i;
...
static int mv88e6095_egress_rate_limiting(struct mv88e6xxx_chip *chip,
					  int port)
{
	return mv88e6xxx_port_write(chip, port, PORT_RATE_CONTROL, 0x0000);
}

static int mv88e6097_egress_rate_limiting(struct mv88e6xxx_chip *chip,
					  int port)
{
 ...
 * remote end.
 */
static int mv88e6097_pause_config(struct mv88e6xxx_chip *chip, int port)
{
...
static int mv88e6xxx_setup_port(struct mv88e6xxx_chip *chip, int port)
{
	...
	if (chip->info->ops->egress_rate_limiting) {
		err = chip->info->ops->egress_rate_limiting(chip, port);
		if (err)
			return err;
	}
...
static int mv88e61xx_get_temp(struct dsa_switch *ds, int *temp)
{
	...
	*temp = 0;
	...
	ret = mv88e6xxx_phy_write(chip, 0x0, 0x16, 0x6, false);
	if (ret < 0)
		goto error;

	/* Enable temperature sensor */
	ret = mv88e6xxx_phy_read(chip, 0x0, 0x1a, &val, false);
	if (ret < 0)
		goto error;

	ret = mv88e6xxx_phy_write(chip, 0x0, 0x1a, val | (1 << 5), false);
	if (ret < 0)
		goto error;
...
error:
	mv88e6xxx_phy_write(chip, 0x0, 0x16, 0x0, false);
	mutex_unlock(&chip->reg_lock);
	return ret;
}
