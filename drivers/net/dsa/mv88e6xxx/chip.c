...
static int mv88e6xxx_led_brightness_set(void * priv, int port,
...
static int mv88e6xxx_led_blink_set(void *priv, int port,
...
static int mv88e6xxx_led_hw_control_is_supported(void *priv, int port, u8 led,
						 unsigned long flags)
{
	struct mv88e6xxx_chip *chip = priv;
	int err;
...
static int mv88e6xxx_led_hw_control_set(void *priv, int port, u8 led,
					unsigned long flags)
{
	struct mv88e6xxx_chip *chip = priv;
	int err;
...
static int mv88e6xxx_led_hw_control_get(void *priv, int port,
...
static struct port_leds_ops mv88e6xxx_port_leds_ops = {
...
static int mv88e6xxx_port_setup(struct dsa_switch *ds, int port)
{
	...
	if (dp->dn) {
		err = port_leds_setup(&dp->user->dev, dp->dn, &chip->leds,
				      chip, port, &mv88e6xxx_port_leds_ops);
		if (err)
...
static void mv88e6xxx_port_teardown(struct dsa_switch *ds, int port)
{
	...
	port_leds_teardown(&chip->leds, port);
