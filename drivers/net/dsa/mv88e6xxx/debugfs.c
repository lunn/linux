...
struct mv88e6xxx_dbg_ops {
	size_t (*read)(struct mv88e6xxx_chip *chip, int id,
		       struct seq_file *seq);
	ssize_t (*write)(struct mv88e6xxx_chip *chip, int id, char *buf,
			 size_t s);
};
...
static size_t mv88e6xxx_dbg_8021q_mode_read(struct mv88e6xxx_chip *chip, int id,
					    struct seq_file *seq)
{
	u16 val;
	int err;
...
static size_t mv88e6xxx_dbg_age_time_read(struct mv88e6xxx_chip *chip, int id,
					  struct seq_file *seq)
{
	unsigned int msecs;
	int err;
...
static ssize_t mv88e6xxx_dbg_age_time_write(struct mv88e6xxx_chip *chip, int id,
...
static size_t mv88e6xxx_dbg_atu_read(struct mv88e6xxx_chip *chip, int id,
				     struct seq_file *seq)
{
...
static ssize_t mv88e6xxx_dbg_atu_write(struct mv88e6xxx_chip *chip, int id,
				       char *buf, size_t size)
{
...
static size_t mv88e6xxx_dbg_default_vid_read(struct mv88e6xxx_chip *chip,
					     int id, struct seq_file *seq)
{
	u16 pvid;
	int err;

	err = _mv88e6xxx_port_pvid_get(chip, id, &pvid);
	if (err)
		return err;
...
static ssize_t mv88e6xxx_dbg_default_vid_write(struct mv88e6xxx_chip *chip,
					       int id, char *buf, size_t size)
{
	u16 pvid;
	...
	return _mv88e6xxx_port_pvid_set(chip, id, pvid);
}
...
static size_t mv88e6xxx_dbg_device_map_read(struct mv88e6xxx_chip *chip, int id,
					    struct seq_file *seq)
{
	u16 val;
	int target, err;
...
static size_t mv88e6xxx_dbg_fid_read(struct mv88e6xxx_chip *chip, int id,
				     struct seq_file *seq)
{
	u16 fid;
	int err;

	err = _mv88e6xxx_port_fid_get(chip, id, &fid);
	if (err)
		return err;
...
static size_t mv88e6xxx_dbg_name_read(struct mv88e6xxx_chip *chip, int id,
				      struct seq_file *seq)
{
	seq_printf(seq, "%s\n", chip->info->name);
...
static size_t mv88e6xxx_pvt_read(struct mv88e6xxx_chip *chip, int src_dev,
...
static size_t mv88e6xxx_pvt_write(struct mv88e6xxx_chip *chip, int src_dev,
				  int src_port, u16 data)
{
	int err;
...
static size_t mv88e6xxx_dbg_pvt_read(struct mv88e6xxx_chip *chip, int id,
				     struct seq_file *seq)
{
	u16 pvlan;
...
static ssize_t mv88e6xxx_dbg_pvt_write(struct mv88e6xxx_chip *chip, int id,
				       char *buf, size_t size)
{
...
static size_t mv88e6xxx_dbg_regs_read(struct mv88e6xxx_chip *chip, int id,
				      struct seq_file *seq)
{
	u16 val;
	int reg;
...
static ssize_t mv88e6xxx_dbg_regs_write(struct mv88e6xxx_chip *chip, int id,
					char *buf, size_t size)
{
...
static size_t mv88e6xxx_dbg_scratch_read(struct mv88e6xxx_chip *chip, int id,
					 struct seq_file *seq)
{
	u16 val;
	int reg, err;
...
static size_t mv88e6xxx_dbg_state_read(struct mv88e6xxx_chip *chip, int id,
				       struct seq_file *seq)
{
	u16 val;
	int err;
...
static size_t mv88e6xxx_dbg_stats_read(struct mv88e6xxx_chip *chip, int id,
				       struct seq_file *seq)
{
	int stat;
	int port;
	...
	if (chip->info->ops->stats_get_stats == mv88e6095_get_stats) {
		types = STATS_TYPE_BANK0 | STATS_TYPE_PORT;
		bank1_select = 0;
		histogram = GLOBAL_STATS_OP_HIST_RX_TX;
	}

	if (chip->info->ops->stats_get_stats == mv88e6320_get_stats) {
		types = STATS_TYPE_BANK0 | STATS_TYPE_BANK1;
		bank1_select = GLOBAL_STATS_OP_BANK_1_BIT_9;
		histogram = GLOBAL_STATS_OP_HIST_RX_TX;
	}

	if (chip->info->ops->stats_get_stats == mv88e6390_get_stats) {
		types = STATS_TYPE_BANK0 | STATS_TYPE_BANK1;
...
static size_t mv88e6xxx_dbg_tcam_read_entry(struct mv88e6xxx_chip *chip,
...
static size_t mv88e6xxx_dbg_tcam_read(struct mv88e6xxx_chip *chip, int id,
				      struct seq_file *seq)
{
	struct mv88e6xxx_tcam_data data;
	int err;
...
static ssize_t mv88e6xxx_dbg_tcam_write(struct mv88e6xxx_chip *chip, int id,
					char *buf, size_t size)
{
	struct mv88e6xxx_tcam_data data;
...
static size_t mv88e6xxx_dbg_tcam_dump_read(struct mv88e6xxx_chip *chip, int id,
					   struct seq_file *seq)
{
	struct mv88e6xxx_tcam_data data;
	int entry = 0;
...
#endif
static size_t mv88e6xxx_dbg_vlan_table_read(struct mv88e6xxx_chip *chip, int id,
					    struct seq_file *seq)
{
	u16 val;
	int i, err;
...
static size_t mv88e6xxx_dbg_vtu_read(struct mv88e6xxx_chip *chip, int id,
				     struct seq_file *seq)
{
	struct mv88e6xxx_vtu_entry entry;
	int err;
...
static ssize_t mv88e6xxx_dbg_vtu_write(struct mv88e6xxx_chip *chip, int id,
				       char *buf, size_t size)
{
...
static size_t mv88e6xxx_dbg_vtu_dump_read(struct mv88e6xxx_chip *chip, int id,
					  struct seq_file *seq)
{
	struct mv88e6xxx_vtu_entry next;
	int err;
