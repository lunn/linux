				  ...
				  u8 const * const data, size_t const nbytes)
{
	...
	dev_dbg(dev, "bootload_data_download() transferred %zd\n", nbytes);
	return 0;
}
			       ...
			       u8 * const data, size_t const nbytes)
{
	...
	udelay(100);
	dev_dbg(dev, "bootload_flash_read() read %zd\n", nbytes);
	return 0;
}
		       ...
		       size_t const nbytes_buf)
{
	...
	if (nbytes_buf != (nwords << 1)) {
		dev_err(dev, "Flash write failed - invalid source buffer size (0x%04zx).\n", nbytes_buf);
		err = -EINVAL;
		goto out;
	}
			...
			size_t const nbytes_buf)
{
	...
	/* Buffer size we're about to write needs to match the segment size. */
	if (nbytes_buf != (nwords << 1)) {
		dev_err(dev, "Flash verify failed - invalid source buffer size (0x%04lx).\n", nbytes_buf);
		err = -EINVAL;
		goto out;
	}
