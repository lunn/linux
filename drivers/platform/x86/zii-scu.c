...
static struct gpiod_lookup_table zii_scu3_mdio_gpiod_table = {
        .dev_id = "mdio-gpio.0",
        .table = {
...
static struct gpiod_lookup_table zii_scu3_gpio_table = {
        .dev_id = "gpio-0",
        .table = {
...
static int __init zii_scu3_mdio_init(void )
{
...
static int __init zii_scu3_init(void)
{
	int err;
	...
	gpiod_add_lookup_table(&zii_scu3_mdio_gpiod_table);
	gpiod_add_lookup_table(&zii_scu3_gpio_table);

	err = zii_scu3_mdio_init();
	if (err) {
...
out_lookup_table:
	gpiod_remove_lookup_table(&zii_scu3_gpio_table);
	gpiod_remove_lookup_table(&zii_scu3_mdio_gpiod_table);

	return err;
}

module_init(zii_scu3_init);
MODULE_LICENSE("GPL");
