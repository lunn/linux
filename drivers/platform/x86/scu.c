...
const char * const ichx_gpiolib_names[128] = {
	[SCU_GPIO_SW_INT] = "switch_interrupt",	/* GPI0 */
	[SCU_GPIO_AC_LOS] = "ac_loss_detect",	/* GPI3 */
	[SCU_GPIO_DBG_OUT] = "debug_out",	/* GPO0 */
	[SCU_GPIO_SW_RST] = "switch_reset",	/* GPO3 */
};
...
static struct gpiod_lookup_table scu_gpios_table = {
	...
	.table = {
		...
		GPIO_LOOKUP_IDX("gpio_ich", SCU_GPIO_AC_LOS,
				NULL, SCU_GPIO_AC_LOS, 0),
		GPIO_LOOKUP_IDX("gpio_ich", SCU_GPIO_DBG_OUT,
				NULL, SCU_GPIO_DBG_OUT, 0),
		GPIO_LOOKUP_IDX("gpio_ich", SCU_GPIO_SW_RST,
				NULL, SCU_GPIO_SW_RST, 0),
		{} /* Terminating entry */
	},
};
...
static void pch_gpio_setup(struct scu_data *data)
{
	...
	gpiod_add_lookup_table(&scu_gpios_table);

	scu_pca953x_pdata[0].irq_base = -1;
	data->pdata->i2c_board_info[0].irq = 0;

	desc = gpiod_get_index(data->dev, NULL, SCU_GPIO_IRQ, 0);
	if (IS_ERR(desc)) {
		dev_err(data->dev, "IRQ missing\n");
		return;
	}

	data->gpio_descs[0] = desc;
	...
	desc = gpiod_get_index(data->dev, NULL, SCU_GPIO_SW_INT, 0);
	if (IS_ERR(desc)) {
		dev_err(data->dev, "Switch interrupt missing\n");
		return;
	}

	data->gpio_descs[1] = desc;
	gpiod_export(desc, false);
	gpiod_direction_input(desc);

	desc = gpiod_get_index(data->dev, NULL, SCU_GPIO_AC_LOS, 0);
	if (IS_ERR(desc)) {
		dev_err(data->dev, "AC LOS missing\n");
		return;
	}

	data->gpio_descs[2] = desc;
	gpiod_export(desc, false);
	gpiod_direction_input(desc);

	desc = gpiod_get_index(data->dev, NULL, SCU_GPIO_DBG_OUT, 0);
	if (IS_ERR(desc)) {
		dev_err(data->dev, "DBG out missing\n");
		return;
	}

	data->gpio_descs[3] = desc;
	gpiod_export(desc, false);
	gpiod_direction_output(desc, 0);

	desc = gpiod_get_index(data->dev, NULL, SCU_GPIO_SW_RST, 0);
	if (IS_ERR(desc)) {
		dev_err(data->dev, "Switch reset missing\n");
		return;
	}

	data->gpio_descs[4] = desc;
	gpiod_export(desc, false);
...
static void pch_gpio_teardown(struct scu_data *data)
{
	...
	for (i = 0; i < ARRAY_SIZE(data->gpio_descs); i++) {
		gpiod_unexport(data->gpio_descs[i]);
		gpiod_put(data->gpio_descs[i]);
	}
}
...
static void scu_setup_ethernet_switch(struct scu_data *data)
{
	if (data->pdata->dsa_data) {
		...
		data->dsa_dev = platform_device_register_data(&platform_bus,
							      "dsa",
							      0, &switch_data,
							      sizeof(switch_data));
...
static struct scu_platform_data scu_platform_data[] = {
	[scu1] = {
		...
		.spi_board_info = scu1_spi_info,
		.num_spi_board_info = ARRAY_SIZE(scu1_spi_info),
	...
	[scu2] = {
		...
		.i2c_board_info = scu_i2c_info_scu2,
		.num_i2c_board_info = ARRAY_SIZE(scu_i2c_info_scu2),
		.spi_board_info = scu2_spi_info,
		.num_spi_board_info = ARRAY_SIZE(scu2_spi_info),
	},
	[scu3] = {
		.board_type = "SCU3 x86",
			       ...
			       struct i2c_board_info *info, int count)
{
	...
	for (i = 0; i < count; i++) {
		data->client[base + i] = i2c_new_client_device(data->adapter, info);
...
static int scu_i2c_adap_name_match(struct device *dev, const void *data)
{
