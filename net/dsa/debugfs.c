				  ...
				  struct seq_file *seq)
{
	...
	stats = kcalloc(count, sizeof(uint64_t), GFP_KERNEL);
	if (!stats) {
		err = -ENOMEM;
		goto out_strings;
	}
...
static int dsa_debugfs_show(struct seq_file *seq, void *p)
{
	...
	//mutex_lock(&ds->dst->lock);
	err = priv->ops->read(ds, priv->id, seq);
	//mutex_unlock(&ds->dst->lock);
