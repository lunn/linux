			...
			struct genl_info *info, const struct nlmsghdr *nlhdr)
{
	...
	if (req_info->req_mask == 0)
		req_info->req_mask = ETHTOOL_IM_RXFLOW_ALL;
			 ...
			 u32 req_mask, const void *_data)
{
	...
	if (!skb)
		goto err_data;
	msg_payload = ethnl_bcastmsg_put(skb, ETHNL_CMD_SET_RXFLOW);
	if (!msg_payload)
		goto err_skb;
...
static const struct nla_policy set_rxflow_policy[ETHTOOL_A_RXFLOW_MAX + 1] = {
	[ETHTOOL_A_RXFLOW_UNSPEC]	= { .type = NLA_REJECT },
	[ETHTOOL_A_RXFLOW_DEV]		= { .type = NLA_NESTED },
...
int ethnl_set_rxflow(struct sk_buff *skb, struct genl_info *info)
{
	...
	ret = set_rxflow_prep_hashfn(dev, info, tb[ETHTOOL_A_RXFLOW_HASH_FN],
				     &hash_fn, &info_mask);
	if (ret < 0)
		goto out_ops;
	ret = set_rxflow_prep_hkey(dev, info, tb[ETHTOOL_A_RXFLOW_HASH_KEY],
				   &hkey, &info_mask);
	if (ret < 0)
		goto out_free;
	ret = set_rxflow_prep_indtbl(dev, info, tb[ETHTOOL_A_RXFLOW_INDIR_TBL],
